...
import { BoardViewConfig, TileViewConfig } from "../configs/UIConfig";
...
export class BoardView extends cc.Component {
  ...
  private tileConfig: TileViewConfig | null = null;
  public applyConfig(cfg?: BoardViewConfig): void {
    if (!cfg) return;
  ...
  public applyTileConfig(cfg?: TileViewConfig): void {
    if (!cfg) return;
    this.tileConfig = cfg;
  ...
  public init(rows: number, cols: number): void {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        if (this.gridLayout) {
          this.gridLayout.applyToCellNode(cell, r, c);
        }
  ...
  public animateViewsToPositions(
    assignments: { view: TileView; to: CellPos }[],
    duration: number = this.moveDuration,
  ): Promise<void> {
  ...
  public moveTilesAnimated(
    moves: { from: CellPos; to: CellPos }[],
    duration: number = this.moveDuration,
  ): Promise<void> {
    if (moves.length === 0) return Promise.resolve();
    return Promise.all(
      moves.map((m) => this.moveTileAnimated(m.from, m.to, duration)),
    ).then(() => {
      this.refreshRenderOrder();
    });
  }
  public swapTilesAnimated(
    a: CellPos,
    b: CellPos,
    duration: number = this.moveDuration,
  ): Promise<void> {
  ...
  private getCellLocalPos(pos: CellPos): cc.Vec2 {
    if (!this.gridLayout) return cc.v2(0, 0);
    return this.gridLayout.getCellLocalPositionByPos(pos);
  }
  private moveTileAnimated(
    from: CellPos,
    to: CellPos,
    duration: number,
  ): Promise<void> {
    ...
    const targetPos = this.getCellLocalPos(to);
  ...
  private bindClick(view: TileView, pos: CellPos): void {
    view.onClick = () => {
      this.onCellClick?.({ r: pos.r, c: pos.c });
    };
  }
}
